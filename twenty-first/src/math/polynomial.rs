    ...
    FF: FiniteField + MulAssign<BFieldElement>,
{
    ...
    pub fn fast_coset_evaluate(&self, offset: BFieldElement, order: usize) -> Vec<FF>
    where
        FF: 'static,
    {
    ...
    FF: FiniteField + MulAssign<BFieldElement>,
{
    ...
    /// - not a power of 2
    /// - larger than [`u32::MAX`]
    pub fn fast_coset_interpolate(offset: BFieldElement, values: &[FF]) -> Self
    where
        FF: 'static,
    {
    ...
    fn naive_coset_extrapolate(
        ...
        intt(&mut coefficients);
        let interpolant = Polynomial::new(coefficients).scale(domain_offset.inverse());
...
impl Polynomial<'_, BFieldElement> {
    ...
    pub fn clean_divide(self, divisor: Self) -> Polynomial<'static, BFieldElement> {
        /// Because [`Polynomial::scale`] is only implemented for offsets in
        /// the base field, the method is duplicated here.
        ///
        /// Returns the coefficient vector of the resulting [`Polynomial`].
        /// This is justified because the context of this function is known
        /// well, since the scope from which this function is accessible is
        /// small.
        fn xfe_scale<FF>(poly: &Polynomial<FF>, offset: XFieldElement) -> Vec<XFieldElement>
        where
            FF: FiniteField + Mul<XFieldElement, Output = XFieldElement>,
        {
            let mut power_of_alpha = XFieldElement::ONE;
            let mut new_coefficients = Vec::with_capacity(poly.coefficients.len());
            for &coefficient in poly.coefficients.iter() {
                new_coefficients.push(coefficient * power_of_alpha);
                power_of_alpha *= offset;
            }
        ...
        let mut dividend_coefficients = xfe_scale(&dividend, offset);
        let mut divisor_coefficients = xfe_scale(&divisor, offset);
        ...
        // to the base field.
        let coeffs = xfe_scale(&quotient, offset.inverse());
