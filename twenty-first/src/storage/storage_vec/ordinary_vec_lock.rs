...
pub struct OrdinaryVecLock<T>(AtomicRw<OrdinaryVecPrivate<T>>);

impl<T> From<Vec<T>> for OrdinaryVecLock<T> {
    fn from(v: Vec<T>) -> Self {
        Self(AtomicRw::from(OrdinaryVecPrivate(v)))
    }
}

impl<T> OrdinaryVecLock<T> {
    #[inline]
...
impl<T> StorageVecRwLock<T> for OrdinaryVecLock<T> {
...
impl<T: Clone> StorageVec<T> for OrdinaryVecLock<T> {
    #[inline]
...
mod tests {
    ...
    mod concurrency {
        ...
        fn gen_concurrency_test_vec() -> OrdinaryVecLock<u64> {
            Default::default()
        }
