...
mod accumulator_mmr_tests {
    ...
    fn root_from_authentication_struct(
        ...
        for i in (0..auth_struct.len()).rev() {
            assert!(nd_auth_struct_indices[i] > prev);
            prev = nd_auth_struct_indices[i];

            let auth_struct_index_as_bfe = node_index_to_bfe(nd_auth_struct_indices[i]);
    ...
    fn root_from_authentication_struct_prop_test(
        #[strategy(0..10usize)] tree_height: usize,
        #[strategy(0usize..100)] _num_revealed_leafs: usize,
        #[strategy(vec(0usize..1<<#tree_height, #_num_revealed_leafs))]
        mut revealed_leaf_indices: Vec<usize>,
    ) {
        ...
        let mut nd_auth_struct_indices = MerkleTree::<Tip5>::authentication_structure_node_indices(
            num_leafs,
        ...
        let nd_siblings = nd_sibling_indices
            .iter()
            .map(|&(l, r)| (tree.node(l).unwrap(), tree.node(r).unwrap()))
            .collect_vec();

        let revealed_leafs = revealed_leaf_indices
            .iter()
            .map(|j| tree.node(*j + num_leafs).unwrap())
            .collect_vec();
        ...
        let nd_auth_struct_indices = nd_auth_struct_indices
            .into_iter()
            .map(|x| x as u64)
            .collect_vec();
        let nd_sibling_indices = nd_sibling_indices
            .into_iter()
            .map(|(x, y)| (x as u64, y as u64))
            .collect_vec();

        let computed_root = root_from_authentication_struct(
            tree_height,
            auth_struct,
            indexed_leafs,
            nd_siblings,
