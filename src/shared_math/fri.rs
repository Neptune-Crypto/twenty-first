...
#[derive(Debug, Clone)]
pub struct Fri<PF: PrimeField, Digest, H> {
...
impl<PF, Digest, H> Fri<PF, Digest, H>
where
    PF: PrimeField + ToDigest<Digest>,
    Digest: ToDigest<Digest> + Clone + PartialEq + Serialize + DeserializeOwned + Sized,
    H: Hasher<Digest = Digest>,
{
    ...
    pub fn prove(
        ...
        // Commit phase
        let merkle_trees: Vec<MerkleTree<PF, Digest, H>> = self.commit(codeword, proof_stream)?;
    ...
    fn commit(
        ...
        codeword: &[PF],
        proof_stream: &mut ProofStream,
    ) -> Result<Vec<MerkleTree<PF, Digest, H>>, Box<dyn Error>> {
    ...
    fn query(
        &self,
        current_mt: MerkleTree<PF, Digest, H>,
        next_mt: MerkleTree<PF, Digest, H>,
        c_indices: &[usize],
        proof_stream: &mut ProofStream,
        ...
        let current_proof: Vec<(LeaflessPartialAuthenticationPath<Digest>, PF)> =
            current_mt.get_leafless_multi_proof_with_values(&ab_indices);

        let next_proof: Vec<(LeaflessPartialAuthenticationPath<Digest>, PF)> =
    ...
    pub fn verify(
        ...
        let first_root: Digest = proof_stream.dequeue_length_prepended::<Digest>()?;
        ...
        for _ in 0..num_rounds {
            ...
            alphas.push(alpha);
            roots.push(proof_stream.dequeue_length_prepended::<Digest>()?);
        }
        ...
        let last_codeword_mt = MerkleTree::<PF, Digest, H>::from_vec(&last_codeword, &zero);
        ...
        for r in 0..num_rounds as usize {
            ...
            let ab_proof: Vec<(LeaflessPartialAuthenticationPath<Digest>, PF)> =
                proof_stream.dequeue_length_prepended()?;
            let c_proof: Vec<(LeaflessPartialAuthenticationPath<Digest>, PF)> =
                proof_stream.dequeue_length_prepended()?;

            // verify Merkle authentication paths
            if !MerkleTree::<PF, Digest, H>::verify_leafless_multi_proof(
                roots[r].clone(),
                &ab_indices,
                &ab_proof,
            ) {
...
mod test_x_field_fri {
    ...
    fn get_rounds_count_test() {
        ...
        let mut fri: Fri<XFieldElement, Digest, Hasher> =
