...
impl<PF: PrimeField> Fri<PF> {
    ...
    pub fn prove(
        ...
        // Commit phase
        let merkle_trees: Vec<MerkleTree<PF>> = self.commit(codeword, proof_stream)?;
    ...
    fn commit(
        ...
        codeword: &[PF],
        proof_stream: &mut ProofStream,
    ) -> Result<Vec<MerkleTree<PF>>, Box<dyn Error>> {
    ...
    fn query(
        &self,
        current_mt: MerkleTree<PF>,
        next_mt: MerkleTree<PF>,
        c_indices: &[usize],
        proof_stream: &mut ProofStream,
        ...
        let current_proof: Vec<(LeaflessPartialAuthenticationPath, PF)> =
            current_mt.get_leafless_multi_proof_with_values(&ab_indices);

        let next_proof: Vec<(LeaflessPartialAuthenticationPath, PF)> =
    ...
    pub fn verify(
        ...
        for r in 0..num_rounds as usize {
            ...
            let ab_proof: Vec<(LeaflessPartialAuthenticationPath, PF)> =
                proof_stream.dequeue_length_prepended()?;
            let c_proof: Vec<(LeaflessPartialAuthenticationPath, PF)> =
                proof_stream.dequeue_length_prepended()?;
