...
mod test_polynomials {
    ...
    fn property_based_slow_lagrange_interpolation_test() {
        ...
        let coefficients: Vec<PrimeFieldElementFlexible> =
            generate_random_numbers(number_of_points, q)
                .iter()
                .map(|x| pfb(*x, q))
                .collect();
    ...
    fn polynomial_arithmetic_property_based_test() {
        ...
        for i in 0..20 {
            let a = Polynomial::<PrimeFieldElementFlexible> {
                coefficients: generate_random_numbers(a_degree, q)
                    .iter()
                    .map(|x| pfb(*x, q))
                    .collect(),
            };
            let b = Polynomial::<PrimeFieldElementFlexible> {
                coefficients: generate_random_numbers(a_degree + i, q)
                    .iter()
                    .map(|x| pfb(*x, q))
                    .collect(),
            };
    ...
    fn fast_multiply_test() {
        let q = 65537;
        let primitive_root = q.get_primitive_root_of_unity(32).0.unwrap();
        ...
        assert_eq!(
            a.shift_coefficients(1, q.ring_zero()),
        ...
        assert_eq!(
            b.shift_coefficients(1, q.ring_zero()),
    ...
    fn fast_evaluate_test() {
        let q = 17;
        let _0_17 = q.ring_zero();
        let _1_17 = q.ring_one();
        let _3_17 = pfb(3, q);
        let _5_17 = pfb(5, q);
    ...
    fn fast_coset_divide_test() {
        let q = 65537;
        let offset = q.get_primitive_root_of_unity(64).0.unwrap();
        let primitive_root = q.get_primitive_root_of_unity(32).0.unwrap();
        println!("primitive_root = {}", primitive_root);
        let a: Polynomial<PrimeFieldElementFlexible> = Polynomial {
            coefficients: vec![
