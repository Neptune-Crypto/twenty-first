...
impl<U: Clone + Debug + Display + DeserializeOwned + PartialEq + Serialize> MimcStarkProof<U> {
    pub fn from_serialization(
        transcript: Vec<u8>,
        start_index: usize,
    ) -> Result<(MimcStarkProof<U>, usize), Box<dyn Error>> {
...
impl MimcStarkProof<BigInt> {
    pub fn verify(
        ...
        let ab_indices: Vec<usize> = self.linear_combination_fri.get_ab_indices(0).unwrap();
...
pub fn stark_of_mimc_prove(
...
) -> Result<MimcStarkProof<BigInt>, StarkProofError> {
    ...
    match low_degree_proof_lc_result {
        ...
        Ok(proof) => {
            println!(
                "Successfully produced proof of low degree ({}) of composition polynomial!",
                max_degree_lc
            );
...
mod test_modular_arithmetic {
    ...
    fn mimc_big_test() {
        ...
        for i in 0..5 {
            ...
            let mut stark_proof: MimcStarkProof<BigInt> = match stark_res {
            ...
            let (deserialized_proof, _) =
                MimcStarkProof::<BigInt>::from_serialization(transcript.clone(), 0).unwrap();
            ...
            // Verify that proof can be deserialized correctly
            let (deserialized_proof, _) =
                MimcStarkProof::<BigInt>::from_serialization(transcript.clone(), 5).unwrap();
            assert_eq!(stark_proof, deserialized_proof);
