...
impl<Value, Digest, H> MerkleTree<Value, Digest, H>
where
    Value: Clone + Serialize + Debug + PartialEq + ToDigest<Digest>,
    Digest: Clone + ToDigest<Digest> + PartialEq,
    H: Hasher<Digest = Digest>,
{
    pub fn verify_proof(root_hash: Digest, index: u64, proof: Vec<Node<Value, Digest>>) -> bool {
    ...
    pub fn get_proof(&self, mut index: usize) -> Vec<Node<Value, Digest>> {
        let mut proof: Vec<Node<Value, Digest>> = Vec::with_capacity(self.height as usize);
    ...
    pub fn get_authentication_path(&self, leaf_index: usize) -> Vec<Digest> {
        let mut auth_path: Vec<Digest> = Vec::with_capacity(self.height as usize);
    ...
    fn get_multi_proof(&self, indices: &[usize]) -> Vec<PartialAuthenticationPath<Value, Digest>> {
        let mut calculable_indices: HashSet<usize> = HashSet::new();
        let mut output: Vec<PartialAuthenticationPath<Value, Digest>> =
            Vec::with_capacity(indices.len());
        for i in indices.iter() {
            let new_branch: PartialAuthenticationPath<Value, Digest> =
    ...
    pub fn get_leafless_multi_proof_with_values(
        &self,
        indices: &[usize],
    ) -> Vec<(LeaflessPartialAuthenticationPath<Digest>, Value)> {
        // auth path and salts
        let leafless_multi_proof: Vec<LeaflessPartialAuthenticationPath<Digest>> =
            self.get_leafless_multi_proof(indices);
        let mut ret: Vec<(LeaflessPartialAuthenticationPath<Digest>, Value)> = vec![];
    ...
    fn get_leafless_multi_proof(
        &self,
        indices: &[usize],
    ) -> Vec<LeaflessPartialAuthenticationPath<Digest>> {
    ...
    fn convert_pat_leafless(
        auth_path_nodes: PartialAuthenticationPath<Value, Digest>,
    ) -> LeaflessPartialAuthenticationPath<Digest> {
        let auth_path_hashes: Vec<Option<Digest>> = auth_path_nodes
            .0
            .iter()
    ...
    fn verify_leafless_multi_proof_from_leaf_hashes(
        ...
        indices: &[usize],
        leaf_hashes: &[Digest],
        proof: &[LeaflessPartialAuthenticationPath<Digest>],
    ) -> bool {
        ...
        let mut partial_auth_paths: Vec<LeaflessPartialAuthenticationPath<Digest>> =
            proof.to_owned();
    ...
    pub fn verify_leafless_multi_proof(
        root_hash: Digest,
        indices: &[usize],
        proof: &[(LeaflessPartialAuthenticationPath<Digest>, Value)],
    ) -> bool {
        ...
        let mut hasher = H::new();
        let mut auth_paths: Vec<LeaflessPartialAuthenticationPath<Digest>> =
    ...
    fn unwrap_leafless_partial_authentication_path(
        partial_auth_path: &LeaflessPartialAuthenticationPath<Digest>,
    ) -> Vec<Digest> {
        partial_auth_path
            .clone()
            .0
...
#[derive(Clone, Debug)]
pub struct SaltedMerkleTree<Value, Digest, H> {
    internal_merkle_tree: MerkleTree<Value, Digest, H>,
    salts: Vec<Value>,
    salts_per_value: usize,
}

impl<Value, Digest, H> SaltedMerkleTree<Value, Digest, H>
where
    Value: Clone + Serialize + Debug + PartialEq + ToDigest<Digest> + GetRandomElements,
    Digest: Clone + ToDigest<Digest> + PartialEq,
    H: Hasher<Digest = Digest>,
{
    ...
    pub fn get_authentication_path(&self, index: usize) -> (Vec<Digest>, Vec<Value>) {
    ...
    pub fn verify_authentication_path(
        ...
        MerkleTree::<Value, Digest, H>::verify_authentication_path_from_leaf_hash(
    ...
    pub fn get_leafless_multi_proof_with_salts_and_values(
        &self,
        indices: &[usize],
    ) -> Vec<(LeaflessPartialAuthenticationPath<Digest>, Vec<Value>, Value)> {
        // auth path and salts
        let leafless_multi_proof: Vec<(LeaflessPartialAuthenticationPath<Digest>, Vec<Value>)> =
            self.get_leafless_multi_proof_with_salts(indices);
        let mut ret: Vec<(LeaflessPartialAuthenticationPath<Digest>, Vec<Value>, Value)> = vec![];
    ...
    pub fn get_leafless_multi_proof_with_salts(
        &self,
        indices: &[usize],
    ) -> Vec<(LeaflessPartialAuthenticationPath<Digest>, Vec<Value>)> {
        // Get the partial authentication paths without salts
        let leafless_partial_authentication_paths: Vec<LeaflessPartialAuthenticationPath<Digest>> =
        ...
        let mut ret: Vec<(LeaflessPartialAuthenticationPath<Digest>, Vec<Value>)> =
            Vec::with_capacity(indices.len());
    ...
    pub fn verify_leafless_multi_proof_with_salts_and_values(
        root_hash: Digest,
        indices: &[usize],
        proof: &[(LeaflessPartialAuthenticationPath<Digest>, Vec<Value>, Value)],
    ) -> bool {
        let values: Vec<Value> = proof.iter().map(|x| x.2.clone()).collect();
        let auth_paths_and_salts: Vec<(LeaflessPartialAuthenticationPath<Digest>, Vec<Value>)> =
    ...
    pub fn verify_leafless_multi_proof(
        ...
        indices: &[usize],
        values: &[Value],
        proof: &[(LeaflessPartialAuthenticationPath<Digest>, Vec<Value>)],
    ) -> bool {
        ...
        let saltless_proof: Vec<LeaflessPartialAuthenticationPath<Digest>> =
            proof.iter().map(|x| x.0.clone()).collect();

        MerkleTree::<Value, Digest, H>::verify_leafless_multi_proof_from_leaf_hashes(
            root_hash,
            indices,
            &leaf_hashes,
            &saltless_proof,
...
mod merkle_tree_test {
    ...
    fn merkle_tree_test_32() {
        ...
        let mut mt_32: MerkleTree<BFieldElement, Digest, Hasher> =
            MerkleTree::from_vec(&elements, &zero);
    ...
    fn merkle_tree_verify_multi_proof_degenerate_test() {
        ...
        let elements: Vec<BFieldElement> = BFieldElement::random_elements(32, &mut rng);
        let zero = BFieldElement::ring_zero();
        let tree = MerkleTree::<BFieldElement, Digest, Hasher>::from_vec(&elements, &zero);
    ...
    fn merkle_tree_verify_multi_proof_equivalence_test() {
        ...
        let zero = BFieldElement::ring_zero();
        let regular_tree = MerkleTree::<BFieldElement, Digest, Hasher>::from_vec(&elements, &zero);
        ...
        let unsalted_salted_tree: SaltedMerkleTree<BFieldElement, Digest, Hasher> =
        ...
        let unsalted_salted_verify =
            SaltedMerkleTree::<BFieldElement, Digest, Hasher>::verify_leafless_multi_proof(
        ...
        for (n_values, expected_path_length) in izip!(n_valuess, expected_path_lengths) {
            let elements: Vec<BFieldElement> =
                generate_random_numbers_u128(*n_values, Some(1u128 << 63))
                    ...
                    .collect();
            let mut tree: MerkleTree<BFieldElement, Digest, Hasher> =
                MerkleTree::<BFieldElement, Digest, Hasher>::from_vec(
                    &elements,
    ...
    fn merkle_tree_test_simple() {
        ...
        let hm = MerkleTree::<BFieldElement, Digest, Hasher>::verify_proof(
            mt.root_hash,
            1,
            proof.clone(),
        );
        ...
        assert!(MerkleTree::<BFieldElement, Digest, Hasher>::verify_proof(
            mt.root_hash,
            0,
            proof.clone()
        ));
    ...
    fn merkle_tree_get_authentication_path_test() {
        ...
        let values_a: Vec<BFieldElement> = vec![3, 6, 9, 12]
            ...
            .collect();
        let zero = BFieldElement::ring_zero();
        let tree_a = MerkleTree::<BFieldElement, Digest, Hasher>::from_vec(&values_a, &zero);
        ...
        let values_b: Vec<BFieldElement> = vec![3, 1, 4, 1, 5, 9, 8, 6]
            ...
            .map(BFieldElement::new)
            .collect();
        let tree_b = MerkleTree::<BFieldElement, Digest, Hasher>::from_vec(&values_b, &zero);
