...
impl<HashDigest, H> Mmr<HashDigest, H> for ArchivalMmr<HashDigest, H>
where
...
mod mmr_test {
    ...
    fn empty_mmr_behavior_test() {
        let mut archival_mmr: ArchivalMmr<blake3::Hash, blake3::Hasher> =
            ArchivalMmr::<blake3::Hash, blake3::Hasher>::new(vec![]);
    ...
    fn bag_peaks_test() {
        ...
        let archival_mmr_small =
            ArchivalMmr::<blake3::Hash, blake3::Hasher>::new(leaf_hashes_blake3.clone());
        ...
        let archival_mmr_small_rp = ArchivalMmr::<Vec<BFieldElement>, RescuePrimeProduction>::new(
        ...
        let archival_mmr_bigger =
            ArchivalMmr::<blake3::Hash, blake3::Hasher>::new(leaf_hashes_bigger_blake3.clone());
    ...
    fn accumulator_mmr_mutate_leaf_test() {
        ...
        for size in 1..150 {
            ...
            let archival_end_state =
                ArchivalMmr::<blake3::Hash, blake3::Hasher>::new(vec![new_leaf; size as usize]);
            for i in 0..size {
    ...
    fn mmr_append_test() {
        ...
        for size in 1..260 {
            let leaf_hashes_blake3: Vec<blake3::Hash> = (500u128..500 + size)
                .map(|x| blake3::hash(bincode::serialize(&x).expect("Encoding failed").as_slice()))
                .collect();
            let mut archival_iterative = ArchivalMmr::<blake3::Hash, blake3::Hasher>::new(vec![]);
            let archival_batch =
                ArchivalMmr::<blake3::Hash, blake3::Hasher>::new(leaf_hashes_blake3.clone());
    ...
    fn variable_size_blake3_mmr_test() {
            ...
            izip!((1u128..34).collect::<Vec<u128>>(), node_counts, peak_counts)
        {
            ...
            let input_hashes: Vec<blake3::Hash> = input_prehashes
                ...
                .map(|x| blake3::hash(bincode::serialize(x).expect("Encoding failed").as_slice()))
                .collect();
            let mut mmr = ArchivalMmr::<blake3::Hash, blake3::Hasher>::new(input_hashes.clone());
            assert_eq!(data_size, mmr.count_leaves());
            assert_eq!(node_count, mmr.count_nodes());
